...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    Vector2f bound_box[2] = {
      {min(min(v[0].x(), v[1].x()), v[2].x()), min(min(v[0].y(), v[1].y()), v[2].y())},
      {max(max(v[0].x(), v[1].x()), v[2].x()), max(max(v[0].y(), v[1].y()), v[2].y())}
    };
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    ...
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        ...
        std::array<Vector3f, 4> zero3f;
        inf.fill(std::numeric_limits<float>::infinity());
        zero3f.fill(Vector3f(0, 0, 0));
        std::fill(depth_buf.begin(), depth_buf.end(), inf);
        std::fill(color_buf.begin(), color_buf.end(), zero3f);
    }
}
