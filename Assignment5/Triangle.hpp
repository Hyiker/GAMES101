...
bool rayTriangleIntersect(const Vector3f &v0, const Vector3f &v1,
  ...
  Vector3f e1 = v1 - v0, e2 = v2 - v0, s = orig - v0,
           s1 = crossProduct(dir, e2), s2 = crossProduct(s, e1);
  float c = 1 / dotProduct(s1, e1); 
  tnear = c * dotProduct(s2, e2);
  u = c * dotProduct(s1, s);
  v = c * dotProduct(s2, dir);
  // use epsilon to eliminate the blue points on the negative edge float
  return (1.0 - u - v) >= -std::numeric_limits<float>::epsilon() && tnear >= 0 && u >= 0 && v >= 0;
}
